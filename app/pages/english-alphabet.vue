...
const pairMode = ref(false); // 大小写成对
...
const letters = computed(() => {
  if (pairMode.value) {
    const pairs: string[] = [];
    uppercaseLetters.forEach((upper, index) => {
      pairs.push(upper);
      pairs.push(lowercaseLetters[index]);
    });
    return pairs.filter((letter) => selectedLetters.value.includes(letter));
  }
...
const lines = computed(() => {
  ...
  letters.value.forEach((letter) => {
    ...
    if (
      ...
      result.length % (letters.value.length / (insertEmptyRows.value + 1)) === 0
    ) {
      result.push('');
    }
  });
...
function toggleLetter(letter: string) {
  const index = selectedLetters.value.indexOf(letter);
  if (index > -1) {
...
function selectAll() {
  selectedLetters.value = [...uppercaseLetters, ...lowercaseLetters];
}
        ...
        <div class="rounded-xl bg-white p-5 shadow-sm">
          <div class="mb-3 flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <span class="font-medium text-slate-800">选择字母</span>
            </div>
            <div class="flex gap-2">
              <button
                class="rounded-lg border border-slate-200 px-2 py-1 text-xs text-slate-600 hover:bg-slate-50"
                @click="selectAll"
              >
                全选
              </button>
          ...
          <div class="mb-3">
            <label class="flex items-center gap-2">
              <input v-model="pairMode" type="checkbox" class="rounded" />
              <span class="text-sm text-slate-700">大小写成对</span>
            </label>
          </div>
        ...
        <div class="rounded-xl bg-slate-100 p-6">
          <div ref="previewRef" class="mx-auto bg-white shadow-lg" :style="previewStyle">
            <div class="mb-6 text-center">
              <h4 class="text-xl font-bold text-slate-800">English Alphabet Practice</h4>
              <p class="mt-1 text-sm text-slate-500">Name: _______ Date: _______</p>
            </div>

            <div class="space-y-2">
              <div
                v-for="(line, index) in lines.slice(0, 30)"
                :key="index"
                class="relative"
                :style="{
